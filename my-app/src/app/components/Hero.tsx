    ...
    <>
      <div className="flex flex-wrap pt-2">
        <div className="flex w-full">
            <div className="lg:w-3/6 w-[50%] sm:p-2 h-full flex items-center justify-center px-4 md:items-start md:justify-start md:p-20 flex-col ">
              ...
              <div className="lg:bg-white flex-col mb-6 w-full md:px-4 py-2 flex sm:flex-row rounded-full items-center justify-center">
                <Search className="text-2xl text-indigo-600 mx-2 hidden sm:flex" />
                <input
                  onChange={(e) => setSearch(e.target.value)}
                  type="text"
                  placeholder="What do you want?"
                  className="xs:w-full w-full rounded-l-full h-full px-2 bg-gray-200 text-base py-3 outline-none"
                />
                <button
                  onClick={handleSearch}
                  className="px-3 py-2 my-2 sm:my-0 border border-indigo-600 uppercase tracking-widest mx-4 rounded-r-full text-white bg-indigo-600 transition-all duration-700 hover:bg-transparent font-semibold text-base hover:text-indigo-600"
                >
                  Search
                </button>
              </div>
            ...
            <div className="hidden lg:flex w-[50%]">
              {/* <Image width={100} height={100} src="/worker.png" alt="hero-img" className="w-full"/> */}
              <img src="/worker.png" alt="" className="object-contain object-cover h-[100vh] w-full"/>
            </div>  
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 p-4 lg:p-0">
          {searchResults &&
